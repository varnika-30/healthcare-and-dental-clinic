...
async function testMappingForPatient(patientId: string) {
  ...
  try {
    ...
    if (appointmentsRes.error) console.error("appointmentsRes error:", appointmentsRes.error);
    if (toothRes.error) console.error("toothRes error:", toothRes.error);
    ...
    console.log("Success! Active plans count:", active.length, "Past plans count:", past.length);
  } catch (err) {
...
async function main() {
  const { data: patients, error } = await supabase.from("patients").select("id");
  if (error) {
    console.error("Failed to fetch patients:", error);
    return;
  }
  const patientIds = (patients || []).map((p) => p.id);
  console.log("Testing mapping for patients:", patientIds);
