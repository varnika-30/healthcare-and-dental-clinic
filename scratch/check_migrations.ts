...
async function main() {
  const { data, error } = await supabase.from("_migrations" as any).select("*");
  console.log("Migrations from _migrations:", { data, error });

  const { data: schemaMigrations, error: schemaError } = await supabase
    .from("schema_migrations" as any)
    .select("*");
  console.log("Schema migrations:", { schemaMigrations, schemaError });
}
