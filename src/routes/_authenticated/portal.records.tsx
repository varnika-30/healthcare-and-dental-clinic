...
function PortalRecords() {
  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!patient) return [];
      const { data } = await supabase.from("tooth_treatments").select("*")
        .eq("patient_id", patient.id).order("performed_at", { ascending: false });
      return data ?? [];
    },
  });

  const marks = (q.data ?? []).map((t) => ({ tooth_number: t.tooth_number, status: t.status, procedure: t.procedure }));
  ...
  return (
    <>
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-muted-foreground">Click a tooth to see procedures and history.</p>
      </div>
              ...
              <div className="space-y-3">
                {(sel ? selectedHistory : q.data ?? []).slice(0, 10).map((t) => (
                  <div key={t.id} className="rounded-xl border border-border/60 p-3 text-sm">
                    <p className="font-medium">Tooth {t.tooth_number} · {t.procedure}</p>
                    <p className="text-xs text-muted-foreground capitalize">{t.status.replace("_", " ")}{t.performed_at ? ` · ${format(new Date(t.performed_at), "MMM d, yyyy")}` : ""}</p>
                    {t.notes && <p className="mt-1 text-xs">{t.notes}</p>}
                  </div>
                ))}
              </div>
            ) : <Empty icon={FileText} title={sel ? "No history for this tooth" : "No procedures yet"} />}
        </Card>
      </div>
    </>
  );
